...
async def test_stream_consumer_fallback_sends_tail_after_partial_overflow():
    ...
    # The first fallback chunk now also carries the Discord save-controls
    # signals: final_response + the COMPLETE logical response (not just the
    # visible tail). Telegram ignores these extra keys.
    assert adapter.send.await_args.kwargs["metadata"] == {
        "thread_id": "77",
        "notify": True,
        "final_response": True,
        "save_response_content": "hello world",
    }
