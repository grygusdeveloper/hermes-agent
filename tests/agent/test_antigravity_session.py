...
def test_stream_drain_timeout_reaps_process_group(monkeypatch):
    monkeypatch.setattr("agent.antigravity_session._PROCESS_DRAIN_GRACE_SECONDS", 0.05)
    conversation = AntigravityConversation()
    process = _blocking_stream_process()
...
def test_durable_state_resumes_after_new_client_without_storing_prompt(
    ...
    state_files = list(
        (tmp_path / "state" / "antigravity-conversations").glob("*.json")
    )
...
def test_full_prompt_accepts_sdk_shaped_tool_calls():
    ...
    assert (
        ...
        '"arguments_json":"{\\"command\\":\\"printf sdk\\"}"}]' in prompt
    )
...
def test_chat_completions_normalization_preserves_responses_provenance():
    ...
    assert history_message["tool_calls"][0]["response_item_id"] == "fc-provider-item"
...
def test_fresh_call_requires_conversation_id(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="did not return a conversation_id"):
        conversation.run("FULL", messages=_messages(("user", "hello")), model="gemini")
...
def test_two_request_clients_resume_one_primary_antigravity_conversation():
    ...
    result = second_client.chat.completions.create(
        model="gemini", messages=second_messages
    )
...
def test_antigravity_abort_terminates_active_process_group(monkeypatch):
    ...
    conversation._active_process = process
    kill_tree = Mock()
    monkeypatch.setattr("agent.antigravity_session.kill_process_tree", kill_tree)

    conversation.abort()

    kill_tree.assert_called_once_with(4321, sig=15)


def test_windows_agy_platform_contract(monkeypatch, tmp_path):
    from agent import antigravity_session

    monkeypatch.setattr(antigravity_session.sys, "platform", "win32")
    monkeypatch.setenv("LOCALAPPDATA", str(tmp_path))
    monkeypatch.setattr(
        "hermes_cli._subprocess_compat.windows_hide_flags",
        Mock(return_value=0x08000000),
    )

    assert antigravity_session._default_agy_path() == str(
        tmp_path / "hermes" / "bin" / "agy.exe"
    )
    assert antigravity_session._agy_popen_platform_kwargs() == {
        "creationflags": 0x08000000
    }


def test_posix_agy_platform_contract(monkeypatch):
    from agent import antigravity_session

    monkeypatch.setattr(antigravity_session.sys, "platform", "linux")
    assert antigravity_session._default_agy_path().endswith(
        str(Path(".local") / "bin" / "agy")
    )
    assert antigravity_session._agy_popen_platform_kwargs() == {
...
def test_abort_during_spawn_publication_is_not_lost(monkeypatch):
    ...
    monkeypatch.setattr("agent.antigravity_session.subprocess.Popen", fake_popen)
    monkeypatch.setattr("agent.antigravity_session.kill_process_tree", fake_kill_tree)
    outcome: list[BaseException] = []
...
def test_late_abort_fails_current_request_without_poisoning_next(monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.antigravity_session.json.loads", abort_during_first_parse
    )
...
def test_small_prompt_still_goes_through_single_execute_call(monkeypatch):
    ...
    conversation.run("small prompt", messages=_messages(("user", "hi")), model="gemini")
...
def test_oversized_incremental_prompt_on_resume_path_is_delivered_as_multiple_turns(
    monkeypatch,
):
    ...
    for previous_call, current_call in zip(calls, calls[1:]):
        assert (
            current_call["conversation_id"]
            == calls[calls.index(previous_call)]["conversation_id"]
            or current_call["conversation_id"]
            == f"cid-{calls.index(previous_call) + 1}"
        )
    assert conversation._conversation_id == f"cid-{len(calls)}"
...
def test_status_error_with_empty_response_still_fails(monkeypatch):
    ...
    conversation = AntigravityConversation()
    payload = json.dumps({"status": "ERROR", "response": "", "conversation_id": None})
    process = SimpleNamespace(
        pid=12345,
        returncode=0,
        communicate=Mock(return_value=(payload, "")),
