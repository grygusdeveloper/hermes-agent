...
compression:
  ...
  provider_threshold_tokens: {claude-code: 250000}  # Per-provider absolute caps, only while that provider is active
...
`provider_threshold_tokens` adds absolute caps that apply only while a given provider is active, so a fallback or `/model` switch to another provider keeps that provider's own threshold; when both caps apply, the lower wins. The default is `{claude-code: 250000}`: the Claude Code CLI re-reads its whole cached conversation on every call and gets markedly slower past ~350k tokens, while its 1M-window models would otherwise compress only at 500k. Setting the map replaces the default; a value of `0` disables a provider's cap.
...
:::tip Cursor Agent
`cursor` uses Cursor's subscription/browser login through the official Cursor Agent CLI (`hermes auth add cursor`). It is not Copilot ACP and not direct xAI billing. If `agent` is not on `PATH`, set the non-secret executable in `cursor.command` (or `providers.cursor.command`). See the [Cursor Agent guide](../guides/cursor-agent.md).
:::
