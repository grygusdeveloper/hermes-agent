...
| `/claude [status\|usage\|context\|models\|stop\|reset\|doctor\|handoff]` | **Claude Code sessions only** (provider `claude-code`; hidden from `/help` elsewhere, native picker on Discord). `status`: model, the Claude-side session and its warm process, the last call's cache hits and API-equivalent cost, plan windows. `usage`: Claude plan limits (5-hour session, weekly, per-model) with resets, asked from Claude Code itself (no credentials read). `context`: Hermes's context meter next to Claude Code's own count. `models`: models Claude Code offers. `stop`: stop the current reply gracefully (the session is kept). `reset`: drop only the Claude-side session; Hermes keeps the conversation and replays it into a fresh one (not mid-turn). `doctor`: CLI binary/version, login state (non-secret fields), bridge state. `handoff`: prints `claude --resume <id> --fork-session` to continue in a terminal. None of these starts a model turn. |
...
| `/usage` | Show token usage, estimated cost breakdown (input/output), context window state, session duration, and — when available from the active provider — an **Account limits** section with remaining quota / credits pulled live from the provider's API. On Claude Code (`claude-code`) it shows the Claude plan windows (5-hour session, weekly, per-model) and their resets; when Claude Code cannot be asked, the limits it last reported with a reply. |
